...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="text",
        help="The mode of optimization, must be one of: 'text', 'image', 'both'",
    )
    parser.add_argument(
        "--reward_model_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Start index of the data to evaluate",
    )
    parser.add_argument(
        "--end_data_idx",
        type=int,
        default=1319,
        help="End index of the data to evaluate",
    )
    parser.add_argument(
        "--task_type", type=str, default="color", help="Type of task for T2I-CompBench"
    )
    ...
    parser.add_argument(
        "--grad_clip", type=float, default=None, help="Gradient clipping threshold"
    )
    parser.add_argument(
        "--text_k",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of text optimization iterations",
    )
    parser.add_argument(
        "--max_image_steps",
        type=int,
        default=10,
        help="Number of image optimization iterations",
    )
    parser.add_argument(
        "--max_both_steps",
        type=int,
        default=10,
        help="Number of both(text and image) optimization iterations",
    )
    parser.add_argument(
        "--max_new_tokens", type=int, default=512, help="Number of generated tokens"
    )
...
def main(args):
    ...
    ### load dataset ###
    dataset = get_meta_milr_opt_dataset(
        args.dataset, args.task_type, args.data_name, args.seed
    )
    ...
    for i in tqdm(data_idx_list):  # Proposal Algorithm line 5
        ...
        if img is not None:
            save_image_and_metadata(
                img, example, os.path.join(output_dir, "ori_img"), i, data_name
            )
        ...
        if new_img is not None:
            save_image_and_metadata(
                new_img, example, os.path.join(output_dir, "opt_img"), i, data_name
            )

        final_img = new_img if new_img is not None else img
        if final_img is not None:
            save_image_and_metadata(
                final_img, example, os.path.join(output_dir, "final_img"), i, data_name
            )
        ...
        fitten_length += (
            (generated_seq - update_length) if len(reward_history) > 1 else 0
        )
